...
export function FacilityMatrixTable({ data }) {
  ...
  const getDataDepth = () => {
    ...
    if (data.columns_tree?.length === 1 && !sampleCounty[data.columns_tree[0]]) {
        ...
        columns.push({
          ...
          cell: ({ getValue }) => (
            <div className="text-center">
              {getValue() !== undefined && getValue() !== null ? getValue().toLocaleString() : '0'}
            </div>
          ),
        });
      });
        ...
        if (header.id === "name" && groupIndex === 0) {
          return (
            <th
              ...
              className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider border border-gray-300 bg-blue-50 sticky left-0 z-10"
          ...
          <Box key={col.id} sx={{ mb: 1 }}>
            ...
            >
              <IconButton size="small">
                ...
                {filterExpandedGroups[col.id] !== false ? <ExpandLessIcon /> : <ExpandMoreIcon />}
              </IconButton>
            ...
            <Collapse in={filterExpandedGroups[col.id] !== false}>
              <Box sx={{ pl: 3, borderLeft: "2px solid #e0e0e0", ml: 1 }}>
                {renderFilterTree(col.columns)} {/* Recursively render nested columns */}
              </Box>
            </Collapse>
          </Box>
        );
  ...
  return (
    <div className="space-y-4 p-4 bg-gray-50 min-h-screen font-inter">
      ...
      >
        <div className="p-4" style={{ width: "320px", maxHeight: "70vh", overflow: "auto" }}>
                  ...
                  {row.getVisibleCells().map(cell => (
                    <td
                      ...
                      className={`px-3 py-4 whitespace-nowrap text-sm text-gray-800 text-center border border-gray-200 ${
                        ...
                        cell.column.id === 'name' ? 'sticky left-0 z-10 bg-white font-medium' : ''
                      }`}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 text-gray-700">
              Page <strong className="font-semibold">{table.getState().pagination.pageIndex + 1}</strong> of{" "}

...
const WardGISMap = ({markerCoordinates, geoJSON, center, ward}) => {
  ...
  return (
        ...
        <>
            ...
            {/* Ward Map */}
            <MapContainer center={ center ?? [-0.44531,  37.1111] } zoom={11.50} maxZoom={15.70} scrollWheelZoom={false} touchZoom={false} style={{ height: '300px', width: "100%", position: 'relative', zIndex: '1', backgroundColor: '#e7eae8', padding: '15px' }}>
                    <TileLayer attribution='&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors' url='http://{s}.tile.osm.org/{z}/{x}/{y}.png' />
                    <GeoJSON data={geoJSON} stylez={geoJsonStyles}/>
                    
                        
                    <Marker position={JSON.parse(window.sessionStorage.getItem('markerCoordinates')) ?? markerCoordinates} >

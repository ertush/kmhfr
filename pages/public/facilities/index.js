...
function Home(props) {
  ...
  function handleSubmit(e) {
    ...
    if (!reset) {
      ...
      const qry = Object.keys(formDataObject)
        .map(function (key) {
          ...
          <div className="w-full md:w-[80%] md:mx-auto px-4 md:px-0 grid grid-cols-1 gap-y-8 md:grid-cols-5 md:gap-4 py-2 md:max-h-min my-4">
            <div className="col-span-1 md:col-span-5 flex flex-col gap-4 md:gap-5 ">
              <div className="flex flex-wrap items-center justify-between gap-2 text-sm md:text-base py-3">
                ...
                <div
                  className={
                    "col-span-5 rounded flex-col md:flex justify-between w-full bg-gray-50  text-black p-4 md:divide-x md:divide-gray-200z items-center border-l-8 " +
                    (true ? "border-gray-600" : "border-red-600")
                  }
                >
                  <h2 className="flex items-center text-xl font-bold text-black capitalize gap-2">
                    {"Facilities"}
                  </h2>
                  <p>Use the filters to search for a facility</p>
                </div>
              </div>
            </div>
            ...
            {/* Side Menu Filters*/}
            <form
              className="col-span-1 max-h-min w-full flex flex-col item-center justify-start md:col-start-1 gap-8"
            ...
            >
              ...
              <div
                className="card col-span-6 md:col-span-2 flex flex-col items-start gap-4 justify-start p-3  shadow-lg border border-gray-300/70 bg-gray-50 rounded"
              ...
              <div
                className="card col-span-6 md:col-span-2 flex flex-col gap-4 items-start justify-start p-3  shadow-lg border border-gray-300/70 bg-gray-50 rounded"
                style={{ minHeight: "50px" }}
              >
                <h2>Facility Info</h2>
              ...
              >
                ...
                <div
                  className="w-full md:max-w-xs flex flex-col gap-4 items-start justify-start mb-3"
                  id="first"
                >
                    ...
                    <div className="w-full" key={i}>
                      <Select
                        ...
                        onChange={(sl) => {
                          ...
                          ct.label == "county" &&
                            sl?.value !== undefined &&
                            getUnits("sub_counties", sl?.value);
                          ct.label == "subcounty" &&
                            sl?.value !== undefined &&
                            getUnits("wards", sl?.value);
                        }}
                      />
                    </div>
                  ))}
              ...
              >
                ...
                >
                  ...
                  <div
                    className="w-full max-w-xs  flex-col items-start justify-start mb-3"
                    id="first"
                  >
                    <input
                    ...
                    <label
                      ...
                      className="text-gray-600 capitalize text-sm ml-1"
                    >
                      {"Has Beds"}
                    </label>
                  </div>
              ...
              >
                <h2>Availability</h2>
                <div
                  className="w-full max-w-xs flex flex-col gap-3 items-start justify-start mb-3"
                  id="first"
                >
                  <div
                    className="w-full max-w-xs  flex-col items-start justify-start mb-3"
            ...
            <div
              className={`${(Array.isArray(facilities) && facilities?.length == 0) || ((!formError || !submitting) && "p-4")} col-span-1 rounded bg-gray-50 md:h-[1670px] md:px-0  p-4 shadow-md md:col-span-4 flex flex-col items-center justify-start md:gap-4 order-last md:order-none`}
            >
              {" "}
              {/* CHANGED colspan */}
              ...
              {formError && (
                <Alert
                  severity="error"
                  className="w-full border-2 border-red-500 rounded-none"
                >
                  {formError}
                </Alert>
              )}
              {submitting && (
                <Alert
                  severity="info"
                  className="w-[95%] mt-3 border-2 border-gray-500 rounded-none"
                >
                  Loading...
                </Alert>
              )}
              <div className="flex flex-col overflow-y-scroll my-4 justify-center items-center md:col-span-4 w-full ">
                <div className="w-full flex justify-end	pt-2 px-1 border-b border-gray-500">
                  <p className="text-end text-gray-500 font-semibold">
                    {props?.facilityCount > 0 ? "30" : "0"} of{" "}
                    {props?.facilityCount}
                  </p>
                </div>
                ...
                facilities?.results.length > 0 ? (
                  facilities?.results.map((hf, index) => (
                    ...
                    >
                      <div className="col-span-8 flex flex-col gap-1 group items-start justify-center  text-left">
                        ...
                        <p className="text-sm text-gray-600 w-full flex gap-2 items-center">
                          {hf?.facility_type_category ? (
                            <span
                              className={
                                "shadow-sm leading-none whitespace-nowrap text-sm  py-1 px-2 bg-blue-200 text-black"
                              }
                            >
                              {hf?.facility_type_category}
                            </span>
                        ...
                        <div className="flex items-center justify-start gap-3">
                          ...
                          <p className="text-sm text-gray-600 w-full flex gap-2 items-center">
                            {(hf?.operational || hf?.operation_status_name) && (
                              <span
                                className={
                                  "shadow-sm leading-none whitespace-nowrap text-sm  py-1 px-2 bg-blue-200 text-black"
                                }
                  ...
                  <div className="w-full px-4 my-6">
                    {!formError && !submitting && (
                      <Alert
                        severity="warning"
                        className="border-2 border-yellow-500 rounded-none"
                      >
                        No facility found{" "}
                      </Alert>
                    )}
...
export async function getServerSideProps(ctx) {
  ...
  const token = (
    await checkToken(ctx.req, ctx.res, {
  ...
  try {
    ...
    const infrastructureOptions = await fetchFilterOptions(
      token,
      "infrastructure",
    );
    const specialitiesOptions = await fetchFilterOptions(token, "specialities");
    const specialityCategoryOptions = await fetchFilterOptions(
      token,

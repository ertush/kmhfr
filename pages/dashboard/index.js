...
const Map = memo(
  dynamic(() => import("../../components/DashboardMap"), {
    loading: () => (
...
function Dashboard(props) {
  ...
  const [facilityTypePresentationType, setFacilityTypePresentationType] =
    useState("bar");
  const [summaryPresentationType, setSummaryPresentationType] = useState("bar");
  const [chuSummaryPresentationType, setCHUSummaryPresentationType] =
    useState("column");
  const [recentChangesPresentationType, setRecentChangesPresentationType] =
    useState("table");
  const [facilityKephPresentationType, setFacilityKephPresentationType] =
    useState("pie");
  const [facilityCHUsPresentationType, setFacilityCHUsPresentationType] =
    useState("bar");
  ...
  async function fetchSubCounties(county) {
    try {
      const r = await fetch(
        `/api/common/fetch_form_data/?path=sub_counties&id=${county}`,
      );
  ...
  async function fetchWards(sub_county) {
    try {
      const r = await fetch(
        `/api/common/fetch_form_data/?path=wards&id=${sub_county}`,
      );
  ...
  const totalSummary = [
    {
      ...
      count: `${props?.data?.total_facilities || 0}`,
    },
    {
      name: "Total approved facilities",
      count: `${props?.data?.approved_facilities || 0}`,
    },
    {
      name: "Total rejected facilities",
      count: `${props?.data?.rejected_facilities_count || 0}`,
    },
    {
      name: "Total closed facilities",
      count: `${props?.data?.closed_facilities_count || 0}`,
    },
    {
      name: "Total facilities pending approval",
      count: `${props?.data?.pending_updates || 0}`,
    },
    {
      name: "Total facilities rejected at validation",
      count: `${props?.data?.facilities_rejected_at_validation || 0}`,
    },
    {
      name: "Total facilities rejected at approval",
      count: `${props?.data?.facilities_rejected_at_approval || 0}`,
    },
  ];
  ...
  const chuSummary = [
    ...
    {
      name: "Total CHUs rejected",
      count: `${props?.data?.rejected_chus || 0}`,
    },
    {
      name: "New CHUs pending approval",
      count: `${props?.data?.recently_created_chus || 0}`,
    },
    {
      name: "Updated CHUs pending approval",
      count: `${props?.data?.chus_pending_approval || 0}`,
    },
  ];
  ...
  const recentChanges = [
    {
      name: "New facilities added",
      count: `${props?.data?.recently_created || 0}`,
    },
    {
      name: "Facilities updated",
      count: `${props?.data?.recently_updated || 0}`,
    },
    {
      name: "New CHUs added",
      count: `${props?.data?.recently_created_chus || 0}`,
    ...
    {
      name: "CHUs updated",
      count: `${props?.data?.recently_updated_chus || 0}`,
    },
  ];
  ...
  function countyOptions(filters, ft) {
    if (groupID === 5 || groupID === 7) {
      let opts = [
        { value: "national", label: "National summary" },
  ...
  function subCountyOptions(filters, ft) {
    if (groupID === 1) {
      let opts = [
        { value: "county", label: "County summary" },
        ...Array.from(subCounties[ft] || [], (fltopt) => {
          if (fltopt.id != null && fltopt.id.length > 0) {
            return {
              value: fltopt.id,
              label: fltopt.name,
  ...
  function wardOptions(filters, ft) {
    if (groupID === 2) {
      let opts = [
        { value: "Subcounty", label: "Subcounty summary" },
        ...Array.from(wards[ft] || [], (fltopt) => {
          if (fltopt.id != null && fltopt.id.length > 0) {
            return {
              value: fltopt.id,
              label: fltopt.name,
  ...
  function handleYearChange(value) {
    ...
    if (recencyRef.current !== null)
      console.log(recencyRef.current.getValue(), county?.childNodes[3]);
      ...
      router.push({
        ...
        query: {
          ...
          ...(() => {
            let result = {};
            const recencyPeriod =
              recencyRef.current?.getValue().length == 1
                ? recencyRef.current?.getValue()[0]?.value
                : null;
  ...
  function handlePresentationChange({ value }, chart_type) {
    ...
    if (chart_type == "facility_type_chart")
      setFacilityTypePresentationType(value);
    if (chart_type == "facility_summary_chart")
      setSummaryPresentationType(value);
    if (chart_type == "chu_summary_chart") setCHUSummaryPresentationType(value);
    if (chart_type == "recent_changes_chart")
      setRecentChangesPresentationType(value);
    if (chart_type == "facility_keph_chart")
      setFacilityKephPresentationType(value);
    if (chart_type == "facility_chu_chart")
      setFacilityCHUsPresentationType(value);
  }

  function defaultPresentation(chart_type) {
    if (chart_type == "owner_chart") {
      chartPresentationOptions.find(
        ({ value }) => value == ownerPresentationType,
      );
    }
    if (chart_type == "facility_type_chart")
      chartPresentationOptions.find(
        ({ value }) => value == facilityTypePresentationType,
      );
    if (chart_type == "facility_summary_chart")
      chartPresentationOptions.find(
        ({ value }) => value == summaryPresentationType,
      );
    if (chart_type == "chu_summary_chart")
      chartPresentationOptions.find(
        ({ value }) => value == chuSummaryPresentationType,
      );
    if (chart_type == "recent_changes_chart")
      chartPresentationOptions.find(
        ({ value }) => value == recentChangesPresentationType,
      );
    if (chart_type == "facility_keph_chart")
      chartPresentationOptions.find(
        ({ value }) => value == facilityKephPresentationType,
      );
    if (chart_type == "facility_chu_chart")
      chartPresentationOptions.find(
        ({ value }) => value == facilityCHUsPresentationType,
      );
  }

  function getTitle() {
    if (groupID == 5 || groupID == 7) {
      // National And Super User groups
      return "National";
    } else if (groupID == 1) {
      // CHRIO Group
        ...
        <MainLayout isLoading={false} searchTerm={props?.query?.searchTerm}>
          <div className="w-full md:w-[85%] md:mx-auto grid grid-cols-1 md:grid-cols-6 gap-3 md:mt-3 md:mb-12 mb-6 px-4 md:px-0">
            <div className="col-span-6 flex flex-col gap-3 md:gap-5 mb-8 ">
              ...
              <div className="no-print flex flex-row gap-2 md:text-base py-3"></div>
                ...
                <div className="w-full flex md:flex-row flex-col gap-4 md:gap-0 justify-between">
                  ...
                  {user && (
                    <div className="w-auto flex items-center gap-3">
                      ...
                      {(groupID == 5 || groupID == 7) &&
                        props?.filters?.county &&
                        props?.filters?.county.length > 0 &&
                      ...
                      {groupID === 1 && (
                        <div className="max-w-min">
                          {Array.isArray(subCounties?.subCounties) &&
                            subCounties?.subCounties.length > 0 &&
                            Object.keys(subCounties)?.map((ft) => {
                              const id = uuid();
                              return (
                                <Select
                                  ...
                                  options={subCountyOptions(filters, ft)}
                                  placeholder={`Select ${ft.split("_").join(" ")[0].toUpperCase() + ft.split("_").join(" ").slice(1)}`}
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    if (props?.query?.county) {
                                      parameters +=
                                        "&county=" + props?.query?.county;
                                    }
                                    if (props?.query?.sub_county) {
                                      parameters +=
                                        "&sub_county=" +
                                        props?.query?.sub_county;
                                    }
                                    if (props?.query?.ward) {
                                      parameters +=
                                        "&ward=" + props?.query?.ward;
                                    }
                                    router.push(
                                      `/dashboard/${encodeURI(parameters)}`,
                                    );
                                  }}
            ...
            <div
              className="card col-span-6 md:col-span-2 flex flex-col items-start justify-start p-3  shadow-lg border border-gray-300/70 bg-gray-50"
              style={{ minHeight: "250px" }}
            >
              <h4 className="text-lg uppercase pt-4 text-center border-b border-gray-100 w-full mb-4 font-semibold text-gray-900">
                Facility owners{" "}
              </h4>
              ...
              {ownerPresentationType !== "table" ? (
                <Chart
                  ...
                  categories={
                    Array?.from(
                      props?.data?.owner_types ?? [],
                      (cs) => cs.name,
                    ) || []
                  }
                  tooltipsuffix="#"
                  xaxistitle={
                    ownerPresentationType.includes("pie") ? null : "Owner Type"
                  }
                  yaxistitle={
                    ownerPresentationType.includes("pie") ? null : "Count"
                  }
                  ...
                  data={
                    (() => {
                      ...
                      data?.push({
                        name: "Facilities",
                        data:
                          Array.from(props?.data?.owner_types ?? [], (cs) => ({
                            name: cs.name,
                            y: parseFloat(cs.count),
                          })) || [],
                      });
                      return data;
                    })() || []
                ...
                <table className="w-full h-full text-sm md:text-base p-2">
                  <thead className="border-b border-gray-300">
                    <tr>
                      <th className="text-left text-gray-800 p-2 text-sm uppercase">
                        Metric
                      </th>
                      <th className="text-right text-gray-800 p-2 text-sm uppercase">
                        Value
                      </th>
                    </tr>
                  </thead>
                  <tbody className="text-lg">
                    {props?.data?.owner_types?.map((ts, i) => (
                      <tr key={i}>
                        <>
                          <td className="table-cell text-left text-gray-900 p-2">
                            {ts.name}
                          </td>
                          <td className="table-cell text-right font-semibold text-gray-900 p-2">
                            {ts.count || 0}
                          </td>
                        </>
                      </tr>
              ...
              <Select
                ...
                onChange={(value) =>
                  handlePresentationChange(value, "owner_chart")
                }
            ...
            >
              ...
              {facilityTypePresentationType !== "table" ? (
                <Chart
                  ...
                  categories={
                    Array?.from(
                      props?.data?.types_summary ?? [],
                      (cs) => cs.name,
                    ) || []
                  }
                  tooltipsuffix="#"
                  xaxistitle={
                    facilityTypePresentationType.includes("pie")
                      ? null
                      : "Facility Type"
                  }
                  yaxistitle={
              ...
              <Select
                ...
                onChange={(value) =>
                  handlePresentationChange(value, "facility_type_chart")
                }
                placeholder="presentation type"
                title="Select Presentation Type"
                className="self-end"
            ...
            >
              ...
              {summaryPresentationType !== "table" ? (
                <Chart
                  title=""
                  categories={
                    Array?.from(totalSummary ?? [], (cs) => cs.name) || []
                  }
                  tooltipsuffix="#"
                  xaxistitle={
                    summaryPresentationType.includes("pie")
                      ? null
                      : "Facility Summaries"
                  }
                  yaxistitle={
                    summaryPresentationType.includes("pie") ? null : "Count"
                  }
              ...
              <Select
                ...
                onChange={(value) =>
                  handlePresentationChange(value, "facility_summary_chart")
                }
                placeholder="presentation type"
                title="Select Presentation Type"
                className="self-end"
            ...
            >
              ...
              {chuSummaryPresentationType !== "table" ? (
                <Chart
                  title=""
                  categories={
                    Array?.from(chuSummary ?? [], (cs) => cs.name) || []
                  }
                  tooltipsuffix="#"
                  xaxistitle={
                    chuSummaryPresentationType.includes("pie")
                      ? null
                      : "Community Unit Summary"
                  }
                  yaxistitle={
                    chuSummaryPresentationType.includes("pie") ? null : "Count"
                  }
              ...
              <Select
                ...
                onChange={(value) =>
                  handlePresentationChange(value, "chu_summary_chart")
                }
                placeholder="presentation type"
                title="Select Presentation Type"
                className="self-end"
            ...
            >
              ...
              {recentChangesPresentationType !== "table" ? (
                <Chart
                  ...
                  tooltipsuffix="#"
                  xaxistitle={
                    recentChangesPresentationType.includes("pie")
                      ? null
                      : "Recent Changes"
                  }
                  yaxistitle={
              ...
              <div className="flex gap-2 self-end">
                ...
                <Select
                  ...
                  onChange={(value) =>
                    handlePresentationChange(value, "recent_changes_chart")
                  }
                  placeholder="presentation type"
                  title="Select Presentation Type"
                />
            ...
            >
                  ...
                  data={
                    (() => {
                      ...
                      data?.push({
                        name: "Facilities",
                        data:
                          Array.from(props?.data?.keph_level ?? [], (cs) => ({
                            name: cs.name,
                            y: parseFloat(cs.count),
                          })) || [],
                      });
              ...
              <Select
                ...
                onChange={(value) =>
                  handlePresentationChange(value, "facility_keph_chart")
                }
                placeholder="presentation type"
                title="Select Presentation Type"
                className="self-end"
            ...
            {(groupID === 7 || groupID === 5) && (
              <div
                className="no-print col-span-6 flex flex-col items-start justify-start p-3  shadow-lg border border-gray-300/70 bg-gray-50"
                style={{ minHeight: "250px" }}
              >
                <h4 className="text-lg uppercase pt-4 border-b text-center border-gray-100 w-full mb-2 font-semibold text-gray-900">
                  ...
                  data={
                    (() => {
                      ...
                      data?.push({
                        name: "Facilities",
                        data:
                          Array.from(props?.data?.county_summary ?? [], (cs) =>
                            parseFloat(cs.count),
                          ) || [],
                      });
                      data?.push({
                ...
                <Select
                  ...
                  options={chartPresentationOptions.filter(
                    ({ value }) => value !== "pie" && value !== "table",
                  )}
                  value={defaultPresentation("facility_chu_chart")}
                  onChange={(value) =>
                    handlePresentationChange(value, "facility_chu_chart")
                  }
                  placeholder="presentation type"
                  title="Select Presentation Type"
                  className="self-end"
            ...
            {groupID === 1 && (
              <div
                ...
                style={{ minHeight: "250px" }}
              >
                <h4 className="text-lg uppercase pt-4 text-center border-b border-gray-100 w-full mb-2 font-semibold text-gray-900">
                ...
                <Chart
                  ...
                  categories={
                    Array?.from(
                      props?.data?.constituencies_summary ?? [],
                      (cs) => cs.name,
                    ) || []
                  }
                  tooltipsuffix="#"
                  ...
                  data={
                    (() => {
                      ...
                      data?.push({
                        name: "Facilities",
                        data:
                          Array.from(props?.data?.wards_summary ?? [], (cs) =>
                            parseFloat(cs.count),
                          ) || [],
                      });
                      data?.push({

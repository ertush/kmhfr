...
const FacilitiesByType = (props) => {
    ...
    const onGridReady = (params) => {
        ...
        const lnlst = props.data.results.map(({type_category, number_of_facilities})=>{return {type_category, number_of_facilities,  actions: ''}})
    ...
    const filter = (e) => {
        e.preventDefault()
        try {
            fetch(`/api/common/submit_form_data/?path=filter_facilities_by_type&drilldown=${JSON.stringify(drillDown)}`, {
				headers:{
			...
			.then(resp =>resp.json())
			.then(res => {
                const results = res.results.map(({type_category, number_of_facilities})=>{return {type_category, number_of_facilities}})
                setUsers(results)

...
const CHUsStatus = (props) => { CHUsStatus
    ...
    const onGridReady = (params) => {
        ...
        const lnlst = props.data.results.map(({id, status_name, number_of_units})=>{return {id, status_name, number_of_units}})
    ...
    const filter = (e) => {
        ...
        try {
			...
			.then(resp =>resp.json())
			.then(res => {
                const results = res.results.map(({id,status_name, number_of_units})=>{return {id, status_name, number_of_units}})
    ...
    useEffect(() => {
        ...
        localStorage.setItem('drill_down', JSON.stringify( drillDown));
    }, [drillDown])
                ...
                <div className="w-full grid grid-cols-7 gap-4 p-1 md:mx-4 my-2">
                    <div className="col-span-7 flex flex-col gap-x-1">
                        <div className="flex flex-wrap items-center justify-between gap-2 text-sm md:text-base py-1">
                            ...
                            <div className={"col-span-5 flex items-center justify-between p-6 w-full bg-transparent drop-shadow  text-black p-4 md:divide-x md:divide-gray-200z items-center border-l-8 " + (true ? "border-blue-600" : "border-red-600")}>
                                <h2 className='flex items-center text-xl font-bold text-black capitalize gap-2'>
                                    {'Community Health Units by Status'}
                                </h2>
                        </div>
                        </div>
                    </div>
                    {/* list */}
                    <ReportsSideMenu  label ={label} />
...
CHUsStatus.getInitialProps = async (ctx) => {
